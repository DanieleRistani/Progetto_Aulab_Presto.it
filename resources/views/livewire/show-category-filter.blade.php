<div class="row  w-75 justify-content-center margin-top-category">
    ...
    <div class="col-12">
           <h2 class="text-center display-1 fw-bold">{{__("ui.$category->name")}}</h2>
    </div>
        ...
        <ul class="navbar-nav">
            <li class="nav-item dropdown mt-5">
                ...
                <ul class="dropdown-menu">
                    <li><button wire:click="filterPriceUp" class="dropdown-item">{{__('ui.prezzofiltro')}} <i class="fa-solid fa-arrow-up ms-3"></i></button></li>
                    <li><button wire:click="filterPriceDown" class="dropdown-item">{{__('ui.prezzofiltro')}} <i class="fa-solid fa-arrow-down ms-3"></i></button></li>
                    ...
                    <nav role="navigation" aria-label="Pagination Navigation" class="d-flex align-items-center justify-content-center ">
                        <span class="mx-2">
                            @if ($articles->onFirstPage())
                                <span class="px-4 py-2 bg-acc text-whitecus rounded-2">
                                    « {{__('ui.indietro')}}
                                </span>
                            @else
                                <button wire:click="previousPage" wire:loading.attr="disabled" rel="prev"
                                    class="px-4 py-2 bg-main btn-custom-sec text-whitecus rounded-2 btn">
                                    « {{__('ui.indietro')}}
                                </button>
                            @endif
                        </span>
    ...
    @else
        <div class="text-center bg-custom-showCategory">
            <h2 class="fw-bold display-2 my-5 ">{{__('ui.noartcategoria')}}</h2>
            @auth
            <a href="{{route('article.create')}}"><button class="btn btn-custom-sec bg-sec fs-2 text-whitecus">{{__('ui.inserisciannuncio')}}</button></a>
            @endauth
